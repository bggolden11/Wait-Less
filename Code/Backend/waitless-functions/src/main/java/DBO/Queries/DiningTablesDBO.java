...
public class DiningTablesDBO implements DBO {
    ...
    public List<DiningTable> getAllDiningTables() throws SQLException {
        ...
        String selectSql = "SELECT Dining_Table_ID, Employee_ID, Seats, Is_Active,Is_Occupied, Seating_Time, Reservation_Name, Has_Birthday,Special_Request "
                + "FROM DiningTable ";
        ...
        ResultSet resultSet = statement.executeQuery(selectSql)) {
            ...
            while(resultSet.next()){
                diningTables.add(new DiningTable(resultSet.getString(1), resultSet.getInt(2),
                                        resultSet.getInt(3),resultSet.getBoolean(4),
                                        resultSet.getBoolean(5),resultSet.getTime(6),
