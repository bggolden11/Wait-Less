...
public class UserService {
   ...
   * @return 200 if user successfully added with new employee Id 500 if internal server error
   */
  public HttpResponseMessage createUser(
    ...
    try {
      ...
      return request
          ...
          .body(new CreateUserResponse(employeeID))
          .build();
   ...
   * @return 200 if valid username and password 401 if invalid password 404 if username not found
    ...
    try {
      ...
      if (userAuthenticationRequest.passwordtoken.equals(password)) {
        ...
        return request
            .createResponseBuilder(HttpStatus.OK)
            .body(
                new UserAuthenticateResponse(
                    userAuthenticationRequest.firstName,
                    userAuthenticationRequest.lastName,
                    userAuthenticationRequest.isManager))
            .build();
      } else
        return request
            .createResponseBuilder(HttpStatus.UNAUTHORIZED)
            .body("Valid user but incorrect password")
            .build();
   ...
   * @return 200 returns the employee information corresponding to that ID 404 if that employee id
   *     was not found 500 if internal server error
   */
  public HttpResponseMessage getUser(
      HttpRequestMessage<Optional<GetEmployeeRequest>> request, String employeeId) {
    try {
      return request
          .createResponseBuilder(HttpStatus.OK)
          .body(userDBO.getEmployee(employeeId))
          .build();
   ...
   * @return 200 log out was successful 500 error connecting to SQL database
   */
  public HttpResponseMessage logUserOut(
      HttpRequestMessage<Optional<LogEmployeeOutRequest>> request, String employeeId) {
    try {
   ...
   * @return 200 list of logged in employees 500 if internal server error
   */
