...
public class FunctionTest {
    ...
    @Test
    public void getTasksStats_proper() throws Exception {

        final HttpRequestMessage<Optional<String>> req = mock(HttpRequestMessage.class);

        final String getTasksStatsRequest = "1001";
        doReturn(Optional.of(getTasksStatsRequest)).when(req).getBody();

        HttpResponseMessage message = new HttpResponseMessageMock.HttpResponseMessageBuilderMock().status(HttpStatus.OK).body(new CreateUserResponse("1234")).build();

        final ExecutionContext context = mock(ExecutionContext.class);

        doReturn(message).when(taskService).getTasksStats(req);

        final HttpResponseMessage ret = function.getTasksStats(req, context);
